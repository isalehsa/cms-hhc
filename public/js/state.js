...
export async function loadAll(force = false) {
  ...
  const [requirements, risks, monitoring, planItems, assessments, findings, cases, visits, training, correspondence, departments, authorities, users, notifications] =
    await Promise.all([
  ...
  Object.assign(store, {
    requirements, risks, monitoring, planItems, assessments, findings, cases, visits, training, correspondence,
...
export async function reload(...cols) {
  const orderFields = { requirements: "code", risks: "code", monitoring: "code", findings: "code", cases: "code", visits: "code", training: "code", correspondence: "code", departments: "name", authorities: "name" };
  await Promise.all(
    cols.map(async (c) => {
